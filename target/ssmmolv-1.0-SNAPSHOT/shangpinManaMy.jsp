...
<head>
    ...
    <script type="text/javascript">
        //导出信息
        function daochu() {
            var url = "${contextPath}/shangpinManaMy?d=1";
        ...
        function shangpinDel(id) {
            if (confirm('您确定删除吗?')) {
                $.get(basePath + '/shangpinDel/' + id, function (resp) {
                    if (resp.success) {
                        alert("删除成功！");
                        //window.location.href = "${contextPath}/shangpinManaMy";
...
<table width="98%" border="0" cellpadding="2" cellspacing="1" bgcolor="#D1DDAA" align="center" style="margin-top:8px"
       bordercolor="#008080">
    <form action="${contextPath}/shangpinManaMy" name="formSearch" id="formSearch" method="post">
...
<%--        用户:
    ...
    <tr bgcolor="#E7E7E7">
        <td height="14" colspan="100" background="${staticPath}/images/tbg.gif">商品管理</td>
    </tr>
    ...
    <c:forEach items="${pageInfo.records}" var="info" varStatus="s">
        <tr align='center' bgcolor="#FFFFFF" onMouseMove="javascript:this.bgColor='red';"
            ...
            <c:choose>
                ...
                <input type="button" value="上架" onclick="shangpinSet(${info.id},'shenhe','yes')"/>
                </c:when>
                <c:otherwise>
                <input type="button" value="下架" onclick="shangpinSet(${info.id},'shenhe','no')"/>
                </c:otherwise>
            </c:choose>
