...
<head>
    ...
    <script type="text/javascript">
        //导出信息
        function daochu() {
            var url = "${contextPath}/gonggaoChakan?d=1";
            window.location.href=url;
        }
        //删除信息
        function gonggaoDel(id) {
            if (confirm('您确定删除吗?')) {
                $.get(basePath + '/gonggaoDel/' + id, function (resp) {
                    if (resp.success) {
                        alert("删除成功！");
                        //window.location.href = "${contextPath}/gonggaoChankan";
        ...
        function gonggaoGet(id) {
            window.location.href = "${contextPath}/gonggaoGet/" + id;
        }

        //跳转到信息详细页
        function gonggaoDetail(id) {
            window.location.href = "${contextPath}/gonggaoDetail/" + id;
        }
        ...
        function gonggaoSet(id,property,value) {
            if (confirm('您确定操作吗?')) {
                ...
                $.post(basePath + '/gonggaoSet', param, function(resp) {
                    if (resp.success) {
                        alert("操作成功！");
                        //window.location.href = "${contextPath}/gonggaoChankan";
                        document.formSearch.submit();
...
<table width="98%" border="0" cellpadding="2" cellspacing="1" bgcolor="#D1DDAA" align="center" style="margin-top:8px"
       bordercolor="#008080">
    <form action="${contextPath}/gonggaoChakan" name="formSearch" id="formSearch" method="post">
        <div>
    ...
    <tr bgcolor="#E7E7E7">
        <td height="14" colspan="100" background="${staticPath}/images/tbg.gif">公告查看</td>
    </tr>
...
<%--                <input type="button" value="修改" onclick="gonggaoGet(${info.id})"/>--%>
                <input type="button" value="详情" onclick="gonggaoDetail(${info.id})"/>
<%--                <input type="button" value="删除" onclick="gonggaoDel(${info.id})"/>--%>
                    <%--sonTable本表子表管理--%>
