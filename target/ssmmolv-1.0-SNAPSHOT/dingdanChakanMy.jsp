...
<head>
    ...
    <script type="text/javascript">
        //导出信息
        function daochu() {
            var url = "${contextPath}/dingdanChakanMy?d=1";
        ...
        function dingdanDel(id) {
            if (confirm('您确定删除吗?')) {
                $.get(basePath + '/dingdanDel/' + id, function (resp) {
                    if (resp.success) {
                        alert("删除成功！");
                        //window.location.href = "${contextPath}/dingdanChankanMy";
...
<table width="98%" border="0" cellpadding="2" cellspacing="1" bgcolor="#D1DDAA" align="center" style="margin-top:8px"
       bordercolor="#008080">
    <form action="${contextPath}/dingdanChakanMy" name="formSearch" id="formSearch" method="post">
...
<%--        用户:
    ...
    <tr bgcolor="#E7E7E7">
        <td height="14" colspan="100" background="${staticPath}/images/tbg.gif">订单查看</td>
    </tr>
    ...
    <c:forEach items="${pageInfo.records}" var="info" varStatus="s">
        <tr align='center' bgcolor="#FFFFFF" onMouseMove="javascript:this.bgColor='red';"
            ...
            <td bgcolor="#FFFFFF" align="center">
            <c:if test="${info.zhifushenhe=='no'}">
                <input type="button" value="确认支付" onclick="dingdanSet(${info.id},'zhifushenhe','yes')"/>
            </c:if>
            <c:if test="${info.shouhuoshenhe=='no'}">
                <input type="button" value="确认收货" onclick="dingdanSet(${info.id},'shouhuoshenhe','yes')"/>
            </c:if>
