...
<head>
    ...
    <script type="text/javascript">
        //导出信息
        function daochu() {
            var url = "${contextPath}/dingdanxiangChakan?d=1";
        ...
        function dingdanxiangDel(id) {
            if (confirm('您确定删除吗?')) {
                $.get(basePath + '/dingdanxiangDel/' + id, function (resp) {
                    if (resp.success) {
                        alert("删除成功！");
                        //window.location.href = "${contextPath}/dingdanxiangChankan";
...
<table width="98%" border="0" cellpadding="2" cellspacing="1" bgcolor="#D1DDAA" align="center" style="margin-top:8px"
       bordercolor="#008080">
    <form action="${contextPath}/dingdanxiangChakan" name="formSearch" id="formSearch" method="post">
...
<%--                <input type="button" value="修改" onclick="dingdanxiangGet(${info.id})"/>--%>
                <c:if test="${null != sessionScope.userType && sessionScope.userType !='0' && info.dingdan.zhifushenhe=='yes' && info.dingdan.shouhuoshenhe=='yes'}">
                    <input type="button" value="添加评论" onclick="shangpinpingjiaAddQian(${info.shangpinid})">
                </c:if>
                <c:if test="${null != sessionScope.userType && sessionScope.userType !='0' && (info.dingdan.zhifushenhe!='yes'|| info.dingdan.shouhuoshenhe!='yes')}">
                    <strong style="color: red;">支付并收货后在此评论</strong>
                </c:if>
